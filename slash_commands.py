...
async def setup_slash_commands(bot, boss_timers, user_stats, user_notifications, table_message, NOTIFICATION_CHANNEL_ID,
    ...
    async def sala_autocomplete(interaction: discord.Interaction, current: str) -> List[app_commands.Choice[int]]:
        try:
            ...
            if not guild_id or guild_id not in boss_timers:
                return [app_commands.Choice(name=f"Sala {i}", value=i) for i in range(1, 9) if current in str(i)][:25]
            
            # Salas padrão + salas adicionais do servidor
            salas = set()
            for boss in boss_timers[guild_id].values():
                salas.update(boss.keys())
            ...
            return [
                ...
                for sala in sorted(salas) if current in str(sala)
            ][:25]
    ...
    async def setup_slash(interaction: discord.Interaction, 
        ...
        try:
            ...
            if canal_notificacao is None:
                canal_notificacao = canal_tabela
    ...
    async def registro_slash(
        ...
        try:
            ...
            # Verificar se o boss existe na estrutura
            if boss_name not in boss_timers[guild_id]:
                boss_timers[guild_id][boss_name] = {}
                for sala_num in range(1, 9):  # Salas padrão
                    boss_timers[guild_id][boss_name][sala_num] = {
                        'death_time': None,
                        'respawn_time': None,
                        'closed_time': None,
                        'recorded_by': None,
            ...
            if not interaction.response.is_done():
                await interaction.response.send_message(
                    f"Ocorreu um erro ao processar seu comando: {str(e)}",
                    ephemeral=True
                )
            else:
                await interaction.followup.send(
    ...
    async def bosshelp_slash(interaction: discord.Interaction):
        ...
        try:
            ...
            embed.add_field(
                ...
                value="Configura os canais do bot neste servidor (apenas admins).\nSe canal_notificacao não for especificado, usa o mesmo canal para ambos.",
                inline=False
            )
